...
Coincidentally the RUDP\footnote{Reliable UDP protocol.} described in \cite{RUDP} suits the needs well on many points. Therefore many of the properties, portrayed in the following parts, are based on those of the RUDP. Note that the RUDP document is an \textit{internet draft}, meaning it is work-in-progress (and seemingly not updated in a long time).
...
\begin{figure}[htb]
 \centering
 \includegraphics[scale=0.62,trim=0 267 0 267]{content/graphics/transport/trans_osi_modelish.pdf}%trim=l b r t
...
\begin{figure}[htb]
 \centering
 \includegraphics[scale=0.64,trim=0 290 0 255]{content/graphics/transport/trans_syn_packburst.pdf}%trim=l b r t
...
\begin{table}[htb]
 ...
 \hline
  \multirow{2}{*}{Header} & \multirow{2}{*}{Unack. seq. $\#1$} & \multirow{2}{*}{Unack. seq. $\#2$} & \multirow{2}{*}{ $\cdots$ } & \multirow{2}{*}{Unack. seq. $\#N$} \\
  & & & & \\
 \hline
 \end{tabular}
 \caption{Transport layer protocol extended acknowledgment packet}
 \label{tab:trans_ext_ack}
\end{table}

In the case there is no data to send, but the user of the protocol wishes to gauge the connection status, a NUL packet (\texttt{NUL} flag set) can be sent. The NUL packet contains no user data, but still consumes sequence numbers normally. When a NUL packet is received, the receiver must respond to it within a specified time interval, or the connection is deemed broken.

The \texttt{RST} flag can be set to send a reset packet. When the reset packet is received, the receiver of the packet must stop sending data and flush it's data I/O queues. This can be used to either reset the connection or close it entirely: If the \texttt{SYN} flag is also set, a reset will occur. A new, randomly generated, sequence number must be attached to the sequence number field in this case. If, on the other hand, the \texttt{SYN} flag is not set, the connection will terminate.
...
\begin{figure}[htb]
 \centering
 \includegraphics[scale=0.64,trim=0 290 0 255]{content/graphics/transport/trans_timeout.pdf}%trim=l b r t
 \caption{The transport layer asdfadsf}
 \label{fig:trans_timeout}
\end{figure}

Figure \ref{fig:trans_timeout} shows a packet with sequence number 20 being sent. It gets lost underway though, and never reaches it's target. Luckily the sender had saved a copy of the sent packet, which he can then resend when the max. retransmission time has passed. When the receiver has later acknowledged the packet it is removed from the queue (this is not shown on the diagram).
