...
This method is used when sending data. It returns an empty \custtt{DtmfOutMessage} message, and enables the user to fill in the message data manually. The message itself contains functionalty to add data. A reference to the message is kept in the \custtt{DtmfApi}, so the message can send itself by the build-in send method. This reference is also used to cleanup unused messages.
