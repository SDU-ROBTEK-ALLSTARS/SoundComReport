...
The main thing the library needs to know when starting is the address of the station on which it has been started, and whether it has the token or not (See data link frame design \ref{dll_frame_design}). These variables can  advantageously be set by the constructor of the class. Because the library needs constant attention on the sound interface to be able to send and receive data, the send and receive logic will be running in a different thread called backbone thread, so the user code continues running in parallel with the Dtmf lib. Threading is described in \ref{threading}.
...
The backbone is described in the chapter \ref{chap:backbone}
...
This method is used when sending data. It returns an empty message, and enables the user to fill ind the message data manually. A reference is kept in the \custtt{DtmfApi}, so the message can be send itself by the build-in send method. This reference is used cleanup unused messages.
